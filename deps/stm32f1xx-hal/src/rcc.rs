...
impl CFGR {
    /// Uses HSE (external oscillator) instead of HSI (internal RC oscillator)
    /// as the clock source. Will result in a hang if an external
    /// oscillator is not connected or it fails to
    /// start.
    ...
    pub fn freeze(self, acr: &mut ACR) -> Clocks {
        ...
        // the USB clock is only valid if an external crystal is used, the PLL is
        // enabled, and the PLL output frequency is a supported one.
        ...
        let adc_prescaler = {
            ...
            if desired >= 6 {
                rcc::cfgr::ADCPREW::DIV8
            } else if desired >= 4 {
                rcc::cfgr::ADCPREW::DIV6
            } else if desired >= 2 {
                rcc::cfgr::ADCPREW::DIV4
            } else {
                rcc::cfgr::ADCPREW::DIV2
            }
        };
...
impl Clocks {
    ...
    /// Returns whether the USBCLK clock frequency is valid for the USB
    /// peripheral
